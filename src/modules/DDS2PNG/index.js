...
module.exports = function convertDDS2Jimp(buf, verbose = false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if ( header[ off_magic ] !== DDS_MAGIC ) {
      ...
      reject( new Error('THREE.DDSLoader.parse: Invalid magic number in DDS header.' ));
      return dds;
    ...
    if ( ! header[ off_pfFlags ] & DDPF_FOURCC ) {
      ...
      reject( new Error('THREE.DDSLoader.parse: Unsupported format, must contain a FourCC code.' ));
      return dds;
    ...
    if ( dds.isCubemap && (
      ...
      reject( new Error('THREE.DDSLoader.parse: Incomplete cubemap faces' ));
      return dds;
    ...
    let data = dds.mipmaps[0].data
    let w = dds.mipmaps[0].width
    let h = dds.mipmaps[0].height
    let uncompressed = dxt.decompress(data, w, h, fourCC === FOURCC_DXT1 ? dxt.flags.DXT1 : dxt.flags.DXT5)
    let bmp = new jimp(w, h)
